...
# Build EGT executable
RUN mkdir /egt
COPY . /egt
RUN cd /egt && mkdir cmake-build-release && cd cmake-build-release \
    && cmake .. \
    && make
...
COPY --from=build /egt/cmake-build-release/commandLineCli /opt/executables/commandLineCli
